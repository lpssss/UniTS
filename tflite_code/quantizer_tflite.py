...
def quantize_model(float_tflite_model_path, calib_data_path, output_model_path, quantization_type):
    ...
    interpreter = tf.lite.Interpreter(model_path=float_tflite_model_path)   

    # Print the signatures from the converted model
    signatures = interpreter.get_signature_list()
    print('Signature:', signatures)

    print(f"Quantizing the model with {quantization_type}...")
    # qt = quantizer.Quantizer(model_path, recipe.dynamic_wi8_afp32())
    if quantization_type == '8w16a':
        qt = quantizer.Quantizer(float_tflite_model_path, recipe.static_wi8_ai16())
    elif quantization_type == '8wdq':
        qt = quantizer.Quantizer(float_tflite_model_path, recipe.dynamic_wi8_afp32())
    elif quantization_type == '8w8a':
        qt = quantizer.Quantizer(float_tflite_model_path, recipe.static_wi8_ai8())
    else:
        raise ValueError("Unsupported quantization type. Use '8w16a' or '8wdq'.")
    ...
    quant_result = qt.quantize(calibration_result=calibration_results).export_model(output_model_path, overwrite=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Quantize a TFLite model using calibration data.")
    parser.add_argument("--float_tflite_model_path", type=str, required=True, help="Path to the float TFLite model.")
    parser.add_argument("--calib_data_path", type=str, required=True, help="Path to the calibration data (npz file).")
    parser.add_argument("--output_model_path", type=str, required=True, help="Path to save the quantized TFLite model.")
    parser.add_argument("--quantization_type", type=str, required=True, choices=['8w16a', '8wdq', '8w8a'], help="Type of quantization to perform.")
    args = parser.parse_args()

    quantize_model(args.float_tflite_model_path, args.calib_data_path, args.output_model_path, args.quantization_type)
